...
class Network:
        ...
        while self.training_epoch < self.config.iter_epoch:
            ...
            try:
                ...
                if self.training_epoch < 5:
                    _, _, summary, l_out, l_h, l_se, probs, labels, acc = self.sess.run(ops, {self.is_training: True, self.se_weight: 0., self.lambda_pl: 1., self.lambda_nl: 0.})
    ...
    def multi_hot_loss(self, labels, logits):
        ...
        pl_labels = labels[:, :, :self.config.num_classes]
    ...
    def gather_valid(self, logits, labels):
        ...
        #one hot label
        sum_labels = tf.reduce_sum(labels, axis=-1, keepdims=False) # [b, n, 13] --> [b, n]
