...
class ModelTester:
    def __init__(self, model, dataset, restore_snap=None, test_smooth=0.05):
        my_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
        self.test_smooth=test_smooth
        self.saver = tf.train.Saver(my_vars, max_to_keep=100)
        self.timeStap = time.strftime('test_%Y-%m-%d_%H-%M-%S', time.gmtime())
        self.smth_str = str(test_smooth).split('.')[0]+str(test_smooth).split('.')[1]+'_'
        self.test_log_path = join(dataset.experiment_dir , 'smth'+self.smth_str+self.timeStap)
    ...
    def test(self, model, dataset, num_votes=100):
        ...
        test_smooth = 1 - self.test_smooth
