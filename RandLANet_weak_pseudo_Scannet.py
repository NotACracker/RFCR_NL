...
class ModelTester:
    ...
    def test(self, model, dataset, num_votes=100):
        ...
        val_proportions = np.zeros(model.config.num_classes, dtype=np.float32)  # [13]
        i = 0
        ...
        while last_min < num_votes:
            try:
                ...
                pred = np.argmax(stacked_probs, 1)
                pred = np.array(idlist)[pred]
                invalid_idx = np.where(stacked_labels == dataset.ignored_labels)[0]
                labels_valid = np.delete(stacked_labels, invalid_idx)
                pred_valid = np.delete(pred, invalid_idx)

                correct = np.sum(pred_valid == labels_valid)
                acc = correct / float(np.prod(np.shape(labels_valid)))
                ...
                stacked_probs = np.reshape(stacked_probs, [model.config.val_batch_size, model.config.num_points,
                                                           model.config.num_classes])  # [B,N,C]
                ...
                if True:
                    ...
                    for i_test in range(num_val):
                        ...
                        preds = dataset.label_values[np.argmax(probs, axis=1)+1].astype(np.int32)  # dataset.label_values：[0,1,...,12]
                        ...
                        label_values = dataset.label_values[1:]
    ...
    def np_normalized(self, z):
        s = np.reshape(np.sum(z, axis=1), [-1,1])
        s[np.argwhere(s <= 0.)] = 1
        a = z / s
        return a