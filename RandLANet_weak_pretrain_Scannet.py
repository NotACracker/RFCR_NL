...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('layers'):
            self.logits, self.se_loss = self.inference(self.inputs, self.is_training)
        ...
        with tf.variable_scope('loss'):
            ...
            self.loss = self.get_loss(valid_logits, valid_labels, self.class_weights) + self.se_loss * self.se_weight
    ...
    def inference(self, inputs, is_training):
        ...
        return f_out, self_enhance_loss
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                if self.training_epoch < 5:
                    _, _, summary, l_out, l_se, probs, labels, acc = self.sess.run(ops, {self.is_training: True, self.se_weight: 0.})
                else:
                    _, _, summary, l_out, l_se, probs, labels, acc = self.sess.run(ops, {self.is_training: True, self.se_weight: 1.})
                self.train_writer.add_summary(summary, self.training_step)
                t_end = time.time()
                if self.training_step % 50 == 0:
                    message = 'Step {:08d} L_out={:5.3f} L_se={:5.3f} Acc={:4.2f} ''---{:8.2f} ms/batch'
                    log_out(message.format(self.training_step, l_out, l_se, acc, 1000 * (t_end - t_start)), self.Log_file)
                self.training_step += 1
