...
class S3DIS:
    def __init__(self, test_area_idx, weak_label, labeled_point):
        self.name = 'S3DIS'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_smooth', type=float, default=0.05, help='test smooth')
    ...
    dataset = S3DIS(test_area, True, FLAGS.labeled_point)
    dataset.init_input_pipeline()
    dataset.experiment_dir, dataset.checkpoints_dir, dataset.tensorboard_log_dir = create_log_dir(FLAGS)

    cfg.saving = False
    model = Network(dataset, cfg)
    snap_path = join(dataset.checkpoints_dir, 'snapshots')
    snap_steps = [int(f[:-5].split('-')[-1]) for f in os.listdir(snap_path) if f[-5:] == '.meta']
    chosen_step = np.sort(snap_steps)[-1]
    chosen_snap = os.path.join(snap_path, 'snap-{:d}'.format(chosen_step))
    tester = ModelTester(model, dataset, restore_snap=chosen_snap, test_smooth=FLAGS.test_smooth)
