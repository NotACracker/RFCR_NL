...
class ConfigS3DIS:
    ...
    noise_init = 3.5  # noise initial parameter
    max_epoch = 100  # maximum epoch during training
    iter_epoch = 30  # iteration epoch during training
    learning_rate = 1e-2  # initial learning rate
    lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate
...
class DataProcessing:
    ...
    def get_class_weights(dataset_name):
        ...
        if dataset_name is 'S3DIS':
            ...
            num_per_class = np.array([13327131, 11989728, 1909991, 1291399, 3122500, 1197818, 1818311, 1942293, 1332850, 1000934,
                                     227936, 244557, 892883, 800339, 246651, 125533, 134002, 110112, 162706, 1575880], dtype=np.int32)
        weight = num_per_class / float(sum(num_per_class))
        ce_label_weight = 1 / np.log(0.02 + weight)
